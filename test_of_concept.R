...
all_dependents <- stock_list$XU100.Index
dependent <- all_dependents[[5]] %>%
    as.data.frame(
        row.names = all_dependents$date,
        col.names = "AKBNK.TI.Equity"
        )
names(dependent) <- "AKBNK.TI.Equity"
...
independent <- market_list$XU100.Index
...
predict.gls <- function(gls_model, newdata, level = 0.95) {
  ...
  if ((class(model_confint) == "try-error") == TRUE) {
    print("Error occured in model specficiation.")
