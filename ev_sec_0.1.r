...
# OBJECTIVE: take data that I have and re-organise into the correct format COMPLICATION: a. For each
# share, their relevant market-proxy is their index.  Therefore, in order to accomplish a
# sectorised estudy I need to fist estimate the securities returns before reorganising.  PROPOSED
# SOLUTION: a. Run the loop to calc the rates and security return (ols) models, Reorganise the
# results according to sectorisation
...
for (i in 1:length(market_list))
{
    tryCatch({
        ...
        securities_returns <- apply_market_model(rates = rates, regressor = rates_indx, same_regressor_for_all = TRUE,
            market_model = "sim", estimation_method = "ols", estimation_start = as.Date("2019-04-01"),
        ...
        ar_results <- dplyr::rename(ar_results, weekday = weekday.x, pct.para = percentage.x, pct.nonpara = percentage.y)
