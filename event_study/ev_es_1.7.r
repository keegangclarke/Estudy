...
# NA CLEANING FUNCTION
clean_na <- function (lyst) {
  # function ingests a list of data.frames
  # It cleans each data.frame of NAs,
  # returning it to the specified format
  for (i in 1:length(lyst)) {
    # selects data.frame in list that is to be manipulated on this iter
    # INPUT
    selector <- lyst[[i]]
    # Make copy of data column in order to handle Nx2 data.frames
...
clean_na(stock_list)
clean_na(market_list)

# CLEANING OF STOCK_LIST
stock_list_copy <- stock_list
# Drop the 'date' columns
for (i in 1:length(stock_list)) {
  stock_list[[i]] <- subset(stock_list[[i]], select = -c(date))
}
# remove 'all NA' ROWS
for (i in 1:length(stock_list)) {
...
# CLEANING OF MARKET_LIST
market_list_copy <- market_list
# Drop the 'date' columns
for (i in 1:length(market_list)) {
  market_list[[i]] <- subset(market_list[[i]], select = -c(date))
}
# remove 'all NA' ROWS
for (i in 1:length(market_list)) {
