...
stock_list_cc <- stock_list_copy
for (i in 1:length(stock_list_cc)) {
  stock_list_cc[[i]] <- subset(stock_list_cc[[i]], select = -c(date))
}
# remove 'all NA' ROWS
for (i in 1:length(stock_list_cc)) {
  selector <- stock_list_cc[[i]]
  selector <- selector[rowSums(is.na(selector)) != ncol(selector), ]
  stock_list_cc[[i]] <- selector
  rm(selector)
}
for (i in 1:length(stock_list_cc)) {
  stock_list_cc[[i]] <- na.omit(stock_list_cc[[i]])
}


# USE OF NA.OMIT MAKES THE DATASET UNUSEABLE 
